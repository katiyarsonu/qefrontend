...
function Sidebar() {
  ...
  // Updated navItems with proper dashboard-based routing from previous examples
  const navItems = [
    ...
    { id: 'extension', label: 'Extension', link: '/dashboard/extension', icon: <FaPuzzlePiece size={20} /> },
    { id: 'support', label: 'Support Center', link: '/dashboard/support', icon: <FaQuestionCircle size={20} /> },
    { id: 'account', label: 'Account', link: '/dashboard/account', icon: <FaUser size={20} /> }
  ]
  ...
  // Desktop sidebar with added Link component for navigation
  const DesktopSidebar = () => (
    <div 
      className={`fixed top-0 left-0 h-full bg-teal-800 text-white transition-all duration-300 z-10 ${
        isSidebarCollapsed ? 'w-16' : 'w-64'
    ...
    >
      ...
      <nav className="mt-4">
        <ul>
          {navItems.map((item) => (
            <li key={item.id}>
              {/* Replaced button with Link for proper routing */}
              <Link
                to={item.link}
                onClick={() => handleItemClick(item.id)}
                className={`w-full flex items-center p-4 hover:bg-teal-700 transition-colors ${
  ...
  // Mobile menu overlay with added Link component
  const MobileMenu = () => (
    <>
      {/* Backdrop */}
      <div 
      ...
      >
        <div className="flex justify-between items-center p-4 border-b border-teal-700">
          <div className="text-xl font-bold tracking-wide">Menu</div>
          <button 
            onClick={toggleMobileMenu}
            className="text-white focus:outline-none"
            aria-label="Close menu"
